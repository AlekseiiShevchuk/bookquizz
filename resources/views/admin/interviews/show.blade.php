...
<li role="presentation" class="active"><a href="#interviewanswers" aria-controls="interviewanswers" role="tab" data-toggle="tab">Interview answers</a></li>
<li role="presentation" class=""><a href="#userinterviewanswers" aria-controls="userinterviewanswers" role="tab" data-toggle="tab">User interview answers</a></li>
</ul>

<!-- Tab panes -->
<div class="tab-content">
    
<div role="tabpanel" class="tab-pane active" id="interviewanswers">
<table class="table table-bordered table-striped {{ count($interview_answers) > 0 ? 'datatable' : '' }}">
    <thead>
        <tr>
    ...
    <tbody>
        @if (count($interview_answers) > 0)
            @foreach ($interview_answers as $interview_answer)
                <tr data-entry-id="{{ $interview_answer->id }}">
                    <td>{{ $interview_answer->text }}</td>
                                <td>
                                    @can('interview_answer_view')
                                    <a href="{{ route('admin.interview_answers.show',[$interview_answer->id]) }}" class="btn btn-xs btn-primary">@lang('quickadmin.qa_view')</a>
                                    @endcan
                                    @can('interview_answer_edit')
                                    <a href="{{ route('admin.interview_answers.edit',[$interview_answer->id]) }}" class="btn btn-xs btn-info">@lang('quickadmin.qa_edit')</a>
                                    @endcan
                                    @can('interview_answer_delete')
                                    {!! Form::open(array(
                                        'style' => 'display: inline-block;',
                                        'method' => 'DELETE',
                                        'onsubmit' => "return confirm('".trans("quickadmin.qa_are_you_sure")."');",
                                        'route' => ['admin.interview_answers.destroy', $interview_answer->id])) !!}
...
<div role="tabpanel" class="tab-pane " id="userinterviewanswers">
<table class="table table-bordered table-striped {{ count($user_interview_answers) > 0 ? 'datatable' : '' }}">
    ...
    <tbody>
        ...
        @endif
            ...
            <a href="{{ route('admin.interviews.index') }}" class="btn btn-default">@lang('quickadmin.qa_back_to_list')</a>
        </div>
    </div>
@stop